...
distanceSPWN1 = 1300;
distanceSPWN2 = 500;
musicON = [true, false] select isMultiplayer;
civPerc = 35;
autoHeal = false;
recruitCooldown = 0;
savingClient = false;
...
Occupants = [independent, west] select (teamPlayer == independent);
Invaders = east;

colourTeamPlayer = ["colorBLUFOR", "colorGUER"] select (teamPlayer == independent);
colorOccupants = ["colorGUER", "colorBLUFOR"] select (teamPlayer == independent);
colorInvaders = "colorOPFOR";

respawnTeamPlayer = ["respawn_west", "respawn_guerrila"] select (teamPlayer == independent);
respawnOccupants = ["respawn_guerrila", "respawn_west"] select (teamPlayer == independent);
...
{
	allMagazines pushBack configName _x;
...
private _alreadyPlaced = [];
private ["_magazines", "_alreadyPlaced", "_weapon", "_weaponType"];
...
] select (!hasRHS && !hasIFA && !myCustomMod);
...
minesAAF = [
	[
		[
			[],
			["LIB_PMD6_MINE_mag","LIB_TM44_MINE_mag","LIB_US_TNT_4pound_mag"]
		] select (hasIFA && !myCustomMod),
		[
			"rhsusf_m112_mag",
			"rhsusf_mine_m14_mag",
			"rhs_mine_M19_mag",
...
itemsAAF = [
	[
		[
			...
			["FirstAidKit","Medikit","ToolKit","LIB_ToolKit"]
		] select (hasIFA && !myCustomMod),
		[
			"FirstAidKit",
			"Medikit",
...
groupsSDKSquadSupp = [
	...
	SDKATman,
	SDKMedic,
	[staticCrewTeamPlayer, staticCrewTeamPlayer],
	[staticCrewTeamPlayer, staticCrewTeamPlayer]
];
...
if (hasRHS) then {
	sniperUnits = sniperUnits + ["rhsusf_socom_marsoc_sniper","rhs_vdv_marksman_asval"];
};
...
civBoats = [
	[],
	["C_Boat_Civil_01_F", "C_Scooter_Transport_01_F", "C_Boat_Transport_02_F", "C_Rubberboat"]
] select !hasIFA;
...
if (!hasIFA) then {
	...
	if (isMultiplayer) then {
		arrayids = arrayids + ["protagonista"];
	};
};
...
injuredSounds = [
	...
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_19_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_20_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_21_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_22_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_23_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_24_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_25_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_26_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_27_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_28_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_29_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_30_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_31_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_32_words.wss",
	"a3\sounds_f\characters\human-sfx\Person1\P1_moan_33_words.wss",
...
ladridos = [
	"Music\dog_bark01.wss",
	"Music\dog_bark02.wss",
	"Music\dog_bark03.wss",
	"Music\dog_bark04.wss",
	"Music\dog_bark05.wss",
	"Music\dog_maul01.wss",
	"Music\dog_yelp01.wss",
	"Music\dog_yelp02.wss",
...
UPSMON_Bld_remove = [
	...
	"containerline_01_f",
	"warehouse_01_f",
	"quayconcrete_01_20m_f",
	"airstripplatform_01_f",
	"airport_02_terminal_f",
	"cliff_wall_long_f",
	"shop_town_05_f",
...
distanceForLandAttack = [3000, 5000] select hasIFA;
...
if (side (group petros) == west) then {
	swoopShutUp pushBack "U_B_Wetsuit";
} else {
	swoopShutUp pushBack "U_I_Wetsuit";
};
...
{server setVariable [_x, 800, true];} forEach [SDKMortar, staticATteamPlayer, staticAAteamPlayer];
//800
...
if (hasIFA) then {
	...
	unlockedItems = unlockedItems + [
		...
		"G_Shades_Black",
		"acc_flashlight"
	] + uniformsSDK + civUniforms;//Initial Arsenal available items

	if (side group petros == independent) then {
		unlockedItems pushBack "I_UavTerminal";
	} else {
		unlockedItems pushBack "B_UavTerminal";
	};
};
...
if (hasIFA) then {
	aceItems append ["ACE_LIB_LadungPM","ACE_SpareBarrel"];
};
...
switch (worldName) do {
	...
	case "chernarus_summer": {
		call compile preprocessFileLineNumbers "roadsDBcherna.sqf";
		{
			...
			["vill_Msta",96],
			["vill_Lopatino",159],
			["city_Zelenogorsk",280],
			["vill_Orlovets",65],
			["city_Berezino",340],
			["vill_Myshkino",49],
			["vill_Petrovka",45],
			["city_Chernogorsk",761],
			["vill_Bor",46],
			["vill_Nizhnoye",146],
			["vill_Balota",147],
			["vill_Khelm",110],
...
if (isMultiplayer) then {
	[petros,"hint","Variables Init Completed"] remoteExec ["A3A_fnc_commsMP", -2];
};
